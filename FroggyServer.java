 ...
 public FroggyServer(TextArea ta){
  ...
  try{
   ...
   if(ACCOUNT_FILE.exists()){
    dis=new DataInputStream(new FileInputStream(ACCOUNT_FILE));
    
    //put accounts into the TreeMap until it runs out of accounts.
    while(true){
     tempAccount=new Account(dis.readUTF(), dis.readUTF(), dis.readUTF());
 ...
 public void save(){
  if(map.size()>0){
   try{
    ...
    dos = new DataOutputStream(new FileOutputStream(ACCOUNT_FILE));
    for(Map.Entry<String, Account> entry : map.entrySet()){
     dos.writeUTF(entry.getValue().getName());
     dos.writeUTF(entry.getValue().getPassword());
     dos.writeUTF(entry.getValue().getColor());
    }
